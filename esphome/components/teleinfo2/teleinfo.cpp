...
static const char *const TAG = "teleinfo2";
...
void TeleInfo::setup() { state_ = WAIT_FRAME; }

void TeleInfo::update() { update_2(); }

void TeleInfo::update_1() {
  if (state_ == OFF) {
    buf_index_ = 0;
    state_ = ON;
  }
...
void TeleInfo::update_2() {
  for (auto &v : values_) {
    publish_value_(v.first, v.second);
  }
}

int TeleInfo::read_chars_until_or_stop_(bool drop, uint8_t c, bool on_stop, int &j) {
  uint8_t received;

  while (available() > 0 && j > 0) {
    j--;
    received = read();
    if (on_stop && (received == 0x3 || received == 0x2))
      return received;
    if (received == c)
      return 1;
    if (drop)
...
void TeleInfo::loop() { loop_2(); }

void TeleInfo::loop_2() {
  uint8_t data;
  int count = 50;
  if (state_ == WAIT_FRAME) {
    if (!read_chars_until_or_stop_(true, 0x2, false, count)) {
      return;
    }
    state_ = WAITING_DATA_SET;
  }
  if (state_ == WAITING_DATA_SET) {
    int res = read_chars_until_or_stop_(true, 0xa, true, count);
    ...
    buf_index_ = 0;
    state_ = GATHER_DATA_SET;
  }
  if (state_ == GATHER_DATA_SET) {
    int res = read_chars_until_or_stop_(true, 0xd, true, count);
    ...
    buf_finger = buf_;
    grp_end = buf_ + buf_index_;
    buf_index_ = 0;
    if (grp_end == buf_finger) {
    ...
    for (auto *element : teleinfo_listeners_) {
      if (tag_ != element->tag)
        continue;
      values_[std::string(tag_)] = std::string(val_);
    }
  }
}

void TeleInfo::loop_1() {
  switch (state_) {
    case OFF:
      break;
    case ON:
